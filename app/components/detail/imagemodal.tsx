...
const ImageModal: React.FC<ImageModalProps> = ({ isOpen, onClose, imageSrc, imageSize }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setScale(1); //minScale);   // If it's a 2X image, start at half size (1024/2048)
    } else {
  ...
  const handleZoomOut = () => setScale(scale => Math.max(scale / 1.25, 1)); //minScale));  // Do not zoom out beyond 1024x1024 equivalent

  if (!shouldRender) return null;

  return (
    <div className={`image-container fixed inset-0 bg-closetoblack flex justify-center items-center z-50 overflow-auto transition-opacity ease-in-out duration-1000 ${showModal ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div style={{ transform: `scale(${scale})` }} className="img-content transition-transform duration-300 cursor-pointer">
        <img src={imageSrc} alt="Enlarged view" /> {/*className="w-auto max-w-none h-auto max-h-none object-contain" />*/}
      </div>
